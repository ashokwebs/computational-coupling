...
**A genuine diagnostic for human relationships, stated carefully.** Not mind-reading — a test for whether two people who *seem* to understand each other (long history, fluent shorthand, finish each other's sentences) are actually still tracking each other's current, specific meaning, versus running on a shared script that used to require attention and now doesn't. Hyperscanning labs already have the data to check this. It would not tell you what someone is thinking. It would tell you whether their partner's specific words are still doing causal work in the exchange, which is a real and different question, and one couples' research has never had a clean way to ask.
...
**None of this is "we will know what you're thinking."** All of it is "we will know, with an actual test, whether the specific thing you meant is the thing that moved the other system" — which sounds smaller and is actually the part that was missing.
