...
1. ~~**Demonstrate the noise-as-instrument route.**~~ **DONE 2026-07-31.** `experiments/paper2_identifiability/noise_as_instrument.py`: a linear-Gaussian coupled/confounded dyad pair calibrated so their observational $\mathrm{Cov}(M,U)$ is identical (naive estimate: $0.800\pm0.005$ vs.\ $0.799\pm0.019$), while the IV estimate using exogenous channel noise as instrument recovers $0.800\pm0.007$ vs.\ $-0.003\pm0.034$ — exact ground truth, 200 seeds. Weak-instrument sweep included (first-stage F-diagnostic) so the honest caveat — this needs real exogenous variance, not just any noise — is itself demonstrated, not asserted.
   **Then applied to the real Stage 2 system, same day.** `noise_as_instrument_stage2.py`: retrained the paper's best config (dual aux, B=8, 20000 episodes; this run's direct-intervention numbers exactly reproduced the paper's own, a good sanity check), then used the Gumbel channel's own transmission noise as instrument on 3000 real rollouts. **The instrument failed: first-stage F=0.2, IV estimate useless (95% CI $[-715, 886]$).** Root cause confirmed by variance decomposition, not just inferred: across repeated noise draws for the same goal, message variance is 99.7% goal-driven and only 0.3% noise-driven — the aux loss that makes the channel informative trains the speaker's logits to confident extremes, which incidentally starves the channel's own noise of relevance as an instrument. A real, diagnosed boundary condition on the method, not a bug and not a refutation of the toy result. `paper2/main.tex` Remark 2 and Limitations updated with both results, recompiled clean (16pp). Full writeup: `experiments/paper2_identifiability/TODO.md`. **Still not done:** a real (non-trained, biological) channel-noise source — a hyperscanning corpus — where this specific saturation failure mode may or may not apply.
