...
- **If compliance is statistically indistinguishable between intact and ablated, while responses remain fluent and superficially on-topic**: this is the headline dissociation result the paper needs, and it would go into `paper2/main.tex` §6.1 (Evaluation of AI systems) replacing the current speculative paragraph with an actual reported finding.
